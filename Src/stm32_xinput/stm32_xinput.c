...
void declareButtonPins(){
	...
	digitalArray[10].port = LED_GPIO_Port;		digitalArray[10].pin = LED_Pin;		digitalArray[10].state = 0;		// XBOX-LOGO
...
void readAdcValues(){
	...
	if( adcValueReady == 1 ){
		...
		xRightStickValue = (int16_t)map( xRightStickValue_ADC, 0, 4040, STICK_16_MAX, STICK_16_MIN );
		yRightStickValue = (int16_t)map( yRightStickValue_ADC, 0, 4040, STICK_16_MIN, STICK_16_MAX );							// 4040 is the max value that my adc presents with potentiometers
		xLeftStickValue = (int16_t)map( xLeftStickValue_ADC, 0, 4040, -48000, 48000 );
		yLeftStickValue = (int16_t)map( yLeftStickValue_ADC, 0, 4040, STICK_16_MAX, STICK_16_MIN  );
